 ...
 */
JNIEXPORT jint JNICALL Java_ml_dmlc_xgboost4j_java_XGBoostJNI_XGBCreateEnclave
(JNIEnv *jenv, jclass jcls) {
  DMatrixHandle result;
  // FIXME parameters 
  char* image = "/root/mc2/code/mc2-xgboost/build/enclave/xgboost_enclave.signed";
  char* usernames[1] = {"user1"};
  int ret = XGBCreateEnclave(image, usernames, 1, 3);
  JVM_CHECK_CALL(ret);
...
JNIEXPORT jint JNICALL Java_ml_dmlc_xgboost4j_java_XGBoostJNI_XGDMatrixCreateFromFile
  ...
  // FIXME user-specified username
  char *username = "user1";
  int ret = XGDMatrixCreateFromFile(fname, username, jsilent, &result);
  JVM_CHECK_CALL(ret);
  if (fname) {
